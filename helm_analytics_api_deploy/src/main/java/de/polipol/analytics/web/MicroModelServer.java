...
public final class MicroModelServer {
		...
		if (app.isPresent()) {
			app.get().routes(() -> {
				if (Utils.isSecured()) {
					...
					before(ENDPOINT_INTERPRETERS, securityHandler);
...
//		});
	...
	public static void startExtendedMode(List<Interpreter> kernels, ModelService<Notebook> notebookService,
			ModelService<Paragraph> paragraphService, ModelService<Filter> filterService, FileService fileService, int port) {
